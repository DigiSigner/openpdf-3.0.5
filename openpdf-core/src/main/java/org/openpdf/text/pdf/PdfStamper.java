 ...
 * PdfContentByte including pages from other Pdfs. The original PDF will keep all the interactive elements including
 * bookmarks, links and form fields.
 * <p>
...
public class PdfStamper
     ...
     * @param os         the output stream
     * @param pdfVersion the new pdf version or '\0' to keep the same version as the original document
     * @param append     if <CODE>true</CODE> appends the document changes as a new revision. This is only useful for
     *                   multiple signatures as nothing is gained in speed or memory
     * @throws DocumentException on error
     * @throws IOException       on error
     */
    ...
    /**
     * Gets the optional <CODE>String</CODE> map to add or change values in the info dictionary.
     *
    ...
    /**
     * An optional <CODE>String</CODE> map to add or change values in the info dictionary. Entries with
    ...
    /**
     * An option to make this stamper to clean metadata in the generated file. You must call this method before closing
     * the stamper.
     */
    ...
    /**
     * Replaces a page from this document with a page from other document. Only the content is replaced not the fields
     * and annotations. This method must be called before getOverContent() or getUndercontent() are called for the same
     * page.
     *
    ...
    /**
     * Inserts a blank page. All the pages above and including <CODE>pageNumber</CODE> will be shifted up. If
     * <CODE>pageNumber</CODE> is bigger than the total number of pages the new page will be the last one.
     *
    ...
    /**
     * Gets a <CODE>PdfContentByte</CODE> to write under the page of the original document.
     *
     * @param pageNum the page number where the extra content is written
     * @return a <CODE>PdfContentByte</CODE> to write under the page of the original document
     */
    ...
    /**
     * Gets a <CODE>PdfContentByte</CODE> to write over the page of the original document.
     *
     * @param pageNum the page number where the extra content is written
     * @return a <CODE>PdfContentByte</CODE> to write over the page of the original document
     */
    ...
    /**
     * Flags the content to be automatically adjusted to compensate the original page rotation. The default is
     * <CODE>true</CODE>.
     *
     ...
     * provided together with an array of the same size for the permissions for each certificate. The open permissions
     * for the document can be AllowPrinting, AllowModifyContents, AllowCopy, AllowModifyAnnotations, AllowFillIn,
     * AllowScreenReaders, AllowAssembly and AllowDegradedPrinting. The permissions can be combined by ORing them.
    ...
    /**
     * Gets a page from other PDF document. Note that calling this method more than once with the same parameters will
    ...
    /**
     * Gets the <CODE>AcroFields</CODE> object that allows to get and set field values and to merge FDF forms.
     *
    ...
    /**
     * Determines if the fields are flattened on close. The fields added with {@link #addAnnotation(PdfAnnotation, int)}
     * will never be flattened.
     *
    ...
    /**
     * Determines if all annotations are flattened on close.
     *
     * @param flat <CODE>true</CODE> to flatten the FreeText annotations, <CODE>false</CODE>
     *             (the default) to keep the FreeText annotations as active content.
     */
    ...
    public void setAnnotationFlattening(final boolean flat) {
        stamper.setAnnotationFlattening(flat);
    }

    /**
     * Adds an annotation of form field in a specific page. This page number can be overridden with
     * {@link PdfAnnotation#setPlaceInPage(int)}.
     *
    ...
    /**
     * Adds a JavaScript action at the document level. When the document opens all this JavaScript runs. The existing
     * JavaScript will be replaced.
     *
    ...
    /**
     * Returns if the file ID property should be included in the PDF file header when creating and signing the PDF
     * file.
     *
     * @return boolean
     * @see PdfName#ID
    ...
    /**
     * Specifies if the file ID property should be included in the PDF file header when creating and stamping the PDF
     * file.
     *
    ...
    /**
     * Returns the enforced PDF file ID, used to specifically override the created PDF file ID when creating and signing
     * the PDF file.
     *
     * @return PdfObject
     * @see PdfName#ID
    ...
    /**
     * Specifies the enforced PDF file ID, used to specifically override the PDF file ID when creating and signing the
     * PDF file.
     *
    ...
    /**
     * Returns the enforced modification date, used to specifically override the PDF modification date (ModDate)
     * property when creating and signing the PDF file.
     *
     * @return Calendar
     * @see PdfName#MODDATE
     * @since OpenPDF 1.2.1
    ...
    /**
     * Specifies the enforced modification date, used to specifically override the PDF modification date (ModDate)
     * property when creating and signing the PDF file.
     *
    ...
    /**
     * Sets whether the /Metadata dictionary should be updated in a new sequential PDF revision update. The condition is
     * applied only when a new revision is being appended (e.g. on a new signature addition). Default : TRUE (/Metadata
     * dictionary is updated in a sequential revision update)
     *
    ...
    /**
     * Sets whether the document /Info dictionary should be updated in a sequential PDF revision update. The condition
     * is applied only when a new revision is being appended (e.g. on a new signature addition). Default : TRUE (/Info
     * dictionary is updated in a sequential revision update)
     *
